...
class HyperClovaLLM:
    ...
    def __init__(self, *, api_key=None, model=None, base_url=None, timeout=None) -> None:
        ...
        self.timeout = float(timeout if timeout is not None else os.getenv("LLM_TIMEOUT", "45"))
...
변수명, JSON, 코드블록, 영문 상태코드 대신 사용자가 이해할 수 있는 한국어 문장으로 설명하세요.
출처는 ENTERPRISE_DOCUMENT 같은 내부 표식 대신 제공된 실제 파일명과 페이지로 적으세요.
