...
from .public_security import public_payload, redact_text
from .source_contract import canonical_route, route_domains, source_usage
...
class ResponseGuard:
    ...
    def _sanitize_answer(value: Any) -> str:
        ...
        text = re.sub(r"\n{3,}", "\n\n", text)
        # Numeric-only paragraphs, dates and tables are user facts, not reasoning.
    ...
    def guard(self, result: dict[str, Any], question_id: str) -> dict[str, Any]:
        ...
        route = canonical_route(result.get("route", ""))
        ...
        if result.get("errors"):
            ...
            error = ErrorEnvelope(error_code=str(last.get("type") or "worker_error"), category="system", node=str(last.get("node") or "worker"), retryable=False, safe_message="요청 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.", internal_reference=str(uuid.uuid4()))
            return public_payload(ResponseEnvelope(status="system_error", answer=error.safe_message, next_action="문제가 반복되면 요청 식별자를 알려주세요.", question_id=question_id, metadata={"route": route, "response_guard_status": "blocked", "error": error.model_dump(mode="json"), **source_usage(result, meta)}).model_dump(mode="json"))
        if clarify:
            status, next_action = "clarify", "필요한 정보를 제공한 뒤 다시 요청하세요."
        elif action_refused:
            status, next_action = "success", "상품 정보나 제도 안내가 필요하면 이어서 질문해 주세요."
            # An execution refusal never carries unverified financial claims.
            answer_text = "실제 매수·주문 체결은 이 상담 채널에서 대행하지 않습니다. 거래는 MTS/HTS 등에서 직접 진행해 주세요."
        elif safe_stop:
        ...
        if status == "success" and not action_refused:
            ...
            elif "document" in route_domains(route) and not has_document:
                failure = "document_source_missing"
            elif "product" in route_domains(route) and not has_product:
                failure = "product_source_missing"
            elif "law" in route_domains(route) and not has_law:
                failure = "law_source_missing"
            elif "calculation" in route_domains(route) and not has_calc:
        ...
        if failure:
            ...
            return public_payload(ResponseEnvelope(status="safe_stop", answer=error.safe_message, limitations=[public_notice(failure)], next_action="조건이나 근거를 확인해 주세요.", question_id=question_id, metadata={"route": route, "response_guard_status": "blocked", "verification_status": verdict, "error": error.model_dump(mode="json"), **source_usage(result, meta)}).model_dump(mode="json"))
        if status == "safe_stop":
            # Never return the old/generated claim under a different status label.
            answer_text = "현재 확인된 근거로는 답변을 안전하게 확정할 수 없습니다. 필요한 조건이나 자료를 추가해 주세요."
        public_sources = [
            {
        ...
        return public_payload(ResponseEnvelope(
            status=status,
            answer=answer_text,
    ...
    def _sources(result: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for item in result.get("results", []) or []:
            sources.append({"domain": "document", "source_file": item.get("filename"), "source_page": item.get("location"), "evidence_id": item.get("chunk_id") or item.get("document_id") or f"document:{item.get('filename')}:{item.get('location')}", "excerpt": str(item.get("text") or "")[:1600]})
        for item in result.get("product_results", []) or []:
            sources.append({"domain": "product", "source_file": item.get("source_file"), "source_page": (item.get("source_pages") or [None])[0], "evidence_id": (item.get("evidence_ids") or [None])[0], "product_id": item.get("record_id") or item.get("product_id"), "excerpt": "\n".join(str(e.get("source_text") or "") for e in item.get("evidence") or [])[:1600]})
        for item in result.get("pdf_evidence") or []:
            if item.get("chunks") or item.get("source_file"):
                excerpt = str(item.get("origin_text") or item.get("text") or "")
                if not excerpt:
                    excerpt = "\n".join(str(chunk.get("text") or "") for chunk in item.get("chunks") or [] if isinstance(chunk, dict))
                sources.append({"domain": "document", "source_file": item.get("source_file"), "source_page": item.get("source_page"), "evidence_id": f"pdf:{item.get('source_file')}:{item.get('source_page')}", "excerpt": excerpt[:1600]})
        law = result.get("law_result") or {}
        for item in [*(law.get("primary_sources") or []), *(law.get("references") or [])]:
            sources.append({"domain": "law", "source_file": item.get("law_name"), "source_page": None, "evidence_id": f"{item.get('law_name')}:{item.get('article_no')}", "source_locator": item.get("article_no"), "excerpt": str(item.get("origin_text") or "")[:1600]})
