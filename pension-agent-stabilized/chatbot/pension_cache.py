...
CACHE_SCHEMA_VERSION = "pension-cache-v2-safety"
ROUTER_POLICY_VERSION = "query-router-v8-source-contract"
...
class SourceVersionTracker:
    ...
    def from_agent(cls, agent: Any) -> "SourceVersionTracker":
        ...
        def law_paths() -> list[Path]:
            repo = root.parent
            legal_db = Path(os.getenv("LEGAL_DB_PATH") or repo / "data/legal/pension_legal.db")
            paths = [
                root / "law_tool.py",
                root / "law_api_client.py",
                root / "law_reference_resolver.py",
                root / "legal_store.py",
                repo / "config/pension_legal_guardrail_v0.1.json",
                legal_db,
            ]
            wal = Path(str(legal_db) + "-wal")
            return paths + ([wal] if wal.is_file() else [])
    ...
    def refresh(self, force: bool = False) -> bool:
        changed = False
        from .legal_store import serving_date
        day = serving_date()
        if getattr(self, "_serving_day", None) != day:
            self._serving_day = day
            force = True
        for field, factory in self._path_factories.items():
            ...
            if force or stamps != self._stamps.get(field):
                ...
                if field == "law_policy" and fingerprint:
                    fingerprint = stable_hash({"content": fingerprint, "as_of": day})
                setattr(self.versions, field, fingerprint)
...
class ToolCacheContext:
    ...
    def law_result(
        ...
        if status == "hit":
            value = deepcopy(value)
            value["external_api_used"] = False
            value["retrieval_cache"] = "hit"
            return value
        result = loader()
