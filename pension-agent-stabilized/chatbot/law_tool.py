...
class LawTool:
    ...
    def search(self, question: str) -> dict[str, Any]:
        # API sessions and counters belong to a request, not the shared web agent.
        request_tool = LawTool.__new__(LawTool)
        request_tool.retriever = self.retriever
        request_tool._api_client = None
        request_tool.allow_api_fallback = self.allow_api_fallback
        try:
            result = request_tool._search_impl(question)
        except Exception as exc:
            result = {"success": False, "primary_sources": [], "references": [], "message": type(exc).__name__, "retrieval_source": "legal_db"}
        finally:
            api = request_tool._api_client
            used = bool(api is not None and getattr(api, "request_count", 0))
            if api is not None:
                api.session.close()
        result["external_api_used"] = used
        return result

    def _search_impl(self, question: str) -> dict[str, Any]:
