...
class RuleVerifier:
    ...
    def verify(
        ...
        # A missing, unresolved, or conflicting required record is never waived
        # merely because another row/document exists.
        ...
        missing_text = [item for item in evidence if item.domain in {"document", "product", "law"} and not str(item.origin_text or "").strip()]
        if missing_text:
            add("evidence_origin_text", "external claims require non-empty source text", "FAIL", "source text", len(missing_text), "An evidence label without source content is not sufficient.", [item.evidence_id for item in missing_text])

        if verification_spec.require_source_version:
            missing_versions = [item for item in evidence if not item.source_version]
            version_ok = bool(evidence) and not missing_versions
        ...
        if "document" in required_domains and not by_domain.get("document"):
            ...
            invalid = [item for item in docs if not item.source_file or item.source_page is None or not str(item.origin_text or "").strip()]
