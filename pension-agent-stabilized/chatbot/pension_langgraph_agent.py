...
class PensionLangGraphAgent:
    ...
    def answer(self, question: str, top_k: int = 5, session_context: dict[str, Any] | None = None, question_id: str | None = None, return_envelope: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            response = {"status": "system_error", "answer": "요청 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.", "sources": [], "limitations": [], "next_action": "문제가 반복되면 요청 식별자를 알려주세요.", "question_id": request.question_id,
                        "metadata": {"error": {"error_code": type(exc).__name__, "node": "graph", "internal_reference": request.question_id}, "response_guard_status": "blocked"}}
            self.audit_backend.write(audit_record(request.question, request.question_id, {}, response, started))
            return response
    ...
    def _product_worker(self, state: PensionAgentState) -> dict[str, Any]:
        return self._delegate_legacy_route(state, state.get("route", "product"))
    ...
    def _rule_verifier_node(self, state: PensionAgentState) -> dict[str, Any]:
        ...
        if result.get("calculation_result"):
            # Compound routes need the same independent arithmetic check.
            calculation = CalculationResult.model_validate(result["calculation_result"])
            calc_report = self.calculation_verifier.verify(
                calculation, [self._evidence_model(item) for item in state.get("evidence", [])]
            )
            report.checks.extend(calc_report.checks)
            report.failures.extend(calc_report.failures)
            report.warnings.extend(calc_report.warnings)
            if calc_report.verdict == "FAIL" or report.verdict == "FAIL":
                report.verdict = "FAIL"
            elif calc_report.verdict == "AMBIGUOUS":
    ...
    def _answer_node(self, state: PensionAgentState) -> dict[str, Any]:
        ...
        collection = state.get("answer_collection")
        if result is None:
            return {"answer_generated": False, "llm_call_count": state.get("llm_call_count", 0)}
        ...
        if not answer:
            direct = collection.get("answer_kind") == "direct"
            if not direct:
                allowed, reservation = self._reserve_llm_call(state, "answer")
                if not allowed or time.time() - state.get("execution_started_at", time.time()) >= self.execution_budget.timeout_seconds:
                    result["answer"] = result["final_answer"] = "실행 예산을 초과해 안전하게 중지했습니다."
        ...
        if not answer:
            if products:
                ...
                result["llm_call_count"] = state.get("llm_call_count", 0) + int(called_llm)
                event = self._record_llm_attempt(reservation, "answer", "failed") if called_llm else {}
                return {**event, "worker_results": {state["route"]: result}, "answer_generated": False, "safe_stop_reason": "answer_provider_failed", "llm_call_count": result["llm_call_count"]}
        ...
        result["llm_call_count"] = state.get("llm_call_count", 0) + int(called_llm)
        event = self._record_llm_attempt(reservation, "answer", "completed") if called_llm else {}
        return {**event, "worker_results": {state["route"]: result}, "answer_generated": True, "llm_call_count": result["llm_call_count"]}
    ...
    def _claim_grounding_node(self, state: PensionAgentState) -> dict[str, Any]:
        ...
        for item in state.get("evidence", []):
            ...
            if isinstance(item, dict) and item.get("status") == "matched" and item.get("origin_text"):
                evidence_texts.append(str(item["origin_text"]))
        verified = (state.get("verification_report") or {}).get("verdict") == "PASS"
        report = self.claim_grounding_verifier.verify(
            answer, evidence_texts, (state.get("calculation_result") or result.get("calculation_result")) if verified else None
        )
        if report.verdict != "PASS":
            blocked = dict(result)
            blocked["answer"] = blocked["final_answer"] = "답변의 수치가 확인된 근거와 일치하지 않아 안전하게 중지했습니다."
            verification = dict(state.get("verification_report") or {})
            verification.update(verdict="FAIL", failures=[*verification.get("failures", []), "unsupported_numeric_claims"])
            return {"claim_grounding_report": report.to_dict(), "worker_results": {state.get("route", "document"): blocked}, "verification_report": verification, "safe_stop_reason": "unsupported_numeric_claims", "answer_generated": False}
    ...
    def _budget_check_node(self, state: PensionAgentState) -> dict[str, Any]:
        ...
        return {"worker_results": {state.get("route", "unknown"): result}, "answer_generated": False, "safe_stop_reason": "execution_budget_exceeded", "llm_call_count": result["llm_call_count"], "execution_budget_applied": True}
    ...
    def _finalize_node(self, state: PensionAgentState) -> dict[str, Any]:
        ...
        # Preserve the verifier's verdict; finalization never upgrades it.
