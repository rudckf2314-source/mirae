...
class LawAPIClient:
    ...
    def get_article(self, law_name: str, article_number: str) -> dict[str, Any] | None:
        ...
        if not normalized:
            return None
        try:
            if not normalized.get("effective_date") or serving_date(str(normalized["effective_date"])) > serving_date():
                return None
        except ValueError:
            return None
        ...
        for article in normalized["articles"]:
            ...
            if compact == target:
                effective = article.get("article_effective_date") or normalized.get("effective_date")
                try:
                    if serving_date(str(effective)) > serving_date():
                        return None
                except ValueError:
                    return None
                return {
