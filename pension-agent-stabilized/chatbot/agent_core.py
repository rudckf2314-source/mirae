...
class PensionAgentCore:
    ...
    def _collect_evidence_base(
        ...
        base = {"question": question, "route": route, "route_reason": decision.reason,
                ...
                "product_lookup_used": False, "document_lookup_used": False,
                "product_backend": getattr(self.product_db, "backend", None)}
        ...
        if route == "document":
            ...
            return {"result": {**base, "results": contexts, "used_tools": ["document"], "document_lookup_used": True},
        ...
        if route == "document+law":
            ...
            text = f"[LAW_EVIDENCE]\n{law_text}\n\n[DOCUMENT_EVIDENCE]\n{document_text}"
            return {"result": {**base, "results": contexts, "law_result": law_result, "used_tools": ["document", "law"], "document_lookup_used": True},
        ...
        if self.product_db.available:
            ...
            if product_results:
                ...
                base["document_lookup_used"] = True
        product_result = {**base, "product_results": product_results, "pdf_evidence": pdf_evidence,
                          "evidence_status": evidence_status, "used_tools": (["product"] if base["product_lookup_used"] else []) + (["document"] if base["document_lookup_used"] else [])}
        product_text = f"[STRUCTURED_DB_EVIDENCE]\n{structured_text}\n\n[PDF_EVIDENCE]\n{pdf_text}"
        if route in {"both", "document+product"}:
            contexts = self.document_chatbot.retriever.retrieve(question, top_k=top_k, source_group=None)
            result = {**product_result, "route": route, "tools": ["document", "product"], "results": contexts, "document_lookup_used": True,
                      "used_tools": ["document"] + (["product"] if base["product_lookup_used"] else [])}
            return {"result": result, "answer_kind": "evidence" if contexts or product_results else None, "contexts": contexts,
                    "evidence_text": product_text + "\n\n[DOCUMENT_EVIDENCE]\n" + self._document_contexts_to_text(contexts)}
        if route == "product":
            named_miss = False
