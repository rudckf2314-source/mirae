...
def collect_trace(
    *,
    ...
    final_result = (state or {}).get("final_result") or state or {}
    usage = meta.get("source_usage") or (meta if "product_lookup_used" in meta else source_usage(final_result))
    route = canonical_route(meta.get("route") or (state or {}).get("route"))
    tools = [name for name, used in (
        ("product", usage.get("product_lookup_used")),
        ("document", usage.get("document_lookup_used")),
        ("law", usage.get("law_lookup_used")),
        ("calculation", bool(final_result.get("calculation_result"))),
    ) if used]
    products = list((state or {}).get("product_results") or final_result.get("product_results") or [])
    docs = list((state or {}).get("results") or final_result.get("results") or [])
    ...
    postgres_used = bool(usage.get("product_lookup_used") and usage.get("backend") == "postgres")
    enterprise = "enterprise_rag" in source_types or bool(docs)
    external = bool(usage.get("external_api_used"))
    ...
    return {
        ...
        "product_backend": usage.get("backend"),
        "structured_product_used": bool(usage.get("product_records_used")),
