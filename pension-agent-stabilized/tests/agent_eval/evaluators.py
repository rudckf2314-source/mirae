...
def source_types_from(envelope: dict[str, Any], state: dict[str, Any] | None) -> list[str]:
    ...
    def _ingest_domain(domain: str) -> None:
        if domain == "document":
            ...
            types.append("structured_product")
        elif domain == "law":
            types.append("law")
    ...
    usage = metadata.get("source_usage") or metadata
    if usage.get("product_lookup_used") and usage.get("backend") == "postgres":
        types.append("postgres")
    if usage.get("external_api_used"):
        types.append("external_api")
    if state:
        if state.get("product_results"):
            types.extend(["product", "structured_product"])
