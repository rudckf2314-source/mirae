"""Gold-100 v2: exact numeric facts, real sources and backend-neutral products.

Historical evaluator is preserved in gold100_evaluator_v1.py. Scores across
versions are not directly comparable; thresholds are unchanged.
"""
...
from chatbot.numeric_facts import contains_fact

EVALUATOR_VERSION = "gold100-v2-exact-source-contract"
...
def number_present(answer: str, token: str) -> bool:
    # Legacy dataset sometimes supplies bare numbers. Match the complete value,
    # never a substring; enforce units whenever the expected token has one.
    return contains_fact(answer, str(token), allow_unspecified_unit=True)
...
def citation_coverage(answer: str, sources: list[dict[str, Any]], trace: dict[str, Any]) -> dict[str, Any]:
    has_source_objs = any(
        item.get("domain") and item.get("evidence_id") and
        (item.get("excerpt") or item.get("source_locator"))
        for item in sources
    )
...
def evaluate_case(
    *,
    ...
    # The historical hint means a structured product source, not one DB vendor.
    if (hints.get("require_postgres") or hints.get("require_structured_product")) and not trace.get("structured_product_used"):
        fails.append("C:structured_product_not_used_for_product_fact")
