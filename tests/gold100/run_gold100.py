...
from tests.gold100.gold100_evaluator import evaluate_case
...
def build_reports(payload: dict[str, Any], turns: list[dict[str, Any]], run_meta: dict[str, Any]) -> dict[str, Any]:
    ...
    for turn in turns:
        ...
        if hints.get("require_enterprise_document") or hints.get("require_postgres"):
            source_total += 1
            ok = True
            if hints.get("require_postgres") and not turn.get("postgres_used"):
                ok = False
...
def main() -> None:
    ...
    for run_idx in range(max(1, args.repeat)):
        ...
        result = build_reports(
            payload,
            turns,
            {"run_id": run_id, "started_at": started_at, "finished_at": finished_at, "executed_count": len(turns)},
        )
        runs.append(result)
