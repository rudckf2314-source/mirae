...
from .answer_contract import is_tax_rule_question, is_product_rule_question, is_investment_limit_rule_question
...
def has_specific_fund_name(question: str) -> bool:
    ...
    if _contains_any(question, FUND_NAME_MARKERS) > 0:
        return True
    # Brand + distinctive product token with a fund-like tail (avoid bare brand hits).
    return bool(
        re.search(
            r"(?:미래에셋|흥국|삼성|한국투자)[가-힣A-Za-z0-9]{2,}(?:투자신탁|펀드|셀렉트\w*|TDF|인덱스)",
            question,
        )
    )
...
class QueryRouter:
    ...
    def decide(self, question: str) -> RouteDecision:
        if is_tax_rule_question(question):
            return RouteDecision(tools=["document", "law"], reason="과세 시점·선택·공제되지 않은 납입금은 제도와 법령을 확인합니다.")
        if is_product_rule_question(question) and not any(word in question for word in ("총보수", "수익률", "비교해", "추천")):
            # Keep product in the toolset so structured-source metadata/route labels remain
            # valid for Gold adapters, while document owns the rule text.
            if is_investment_limit_rule_question(question):
                return RouteDecision(tools=["document", "product"], reason="운용제한·한도는 문서 규칙을 보고 구조화 상품 경로도 함께 표시합니다.")
            return RouteDecision(tools=["document"], reason="상품의 절차·운용규칙·한도 적용을 확인합니다.")
        if is_investment_limit_rule_question(question):
            return RouteDecision(tools=["document", "product"], reason="운용제한·파생·사모 한도는 문서 규칙과 구조화 상품 경로를 함께 사용합니다.")
        ...
        if intent.primary in {"procedure", "institution"} and not (named_product and search_hits):
            ...
            if named_product and any(word in question for word in ("벤치마크", "benchmark", "비교지수", "보수", "수익률", "클래스", "편입비율", "MSCI")):
                return RouteDecision(tools=["product"], reason="특정 상품의 전제는 구조화 상품 사실부터 확인합니다.")
            if is_investment_limit_rule_question(question):
                return RouteDecision(tools=["document", "product"], reason="파생·사모 한도 전제는 문서 규칙과 구조화 상품 경로를 확인합니다.")
