...
class LegalStore:
    ...
    def get_article(self, source_key: str, article_no: str, as_of: str | None = None) -> dict[str, Any] | None:
        params: list[Any] = [source_key, article_no]
        clause = ""
        if as_of:
            clause = " AND (effective_date IS NULL OR effective_date <= ?)"
            params.append(as_of)
        with self.connect() as conn:
            row = conn.execute(
                f"""
                SELECT * FROM legal_articles
                 WHERE source_key=? AND article_no=? {clause}
    ...
    def get_articles_for_sources(
        ...
        for raw in rows:
            ...
            if allowed and item["article_no"] not in allowed:
                continue
            if q_tokens:
    ...
    def get_policy_rule(self, policy_key: str, policy_year: int) -> dict[str, Any] | None:
        with self.connect() as conn:
            row = conn.execute(
                """
